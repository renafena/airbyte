...
class SnowflakeSqlGenerator(private val retentionPeriodDays: Int) : SqlGenerator {
    ...
    override fun createSchema(schema: String): Sql {
        return Sql.of(
            StringSubstitutor(
                    java.util.Map.of<String, String>("schema", StringUtils.wrap(schema, QUOTE))
                )
    ...
    override fun updateTable(
        ...
        return Sql.transactionally(insertNewRecords, dedupFinalTable, cdcDeletes, commitRawTable)
    }
                ...
                .map<String>(
                    Function<Map.Entry<ColumnId, AirbyteType>, String> {
                        ...
                        StringSubstitutor(
                            ...
                            .replace( // TYPEOF returns "NULL_VALUE" for a JSON null and "NULL" for
                                // a SQL null
                                """
                CASE
    ...
    override fun prepareTablesForSoftReset(stream: StreamConfig): Sql {
        return Sql.concat(
            createTable(
                stream,
                TyperDeduperUtil.SOFT_RESET_SUFFIX.uppercase(Locale.getDefault()),
                true
            ),
    ...
    override fun clearLoadedAt(streamId: StreamId): Sql {
        return Sql.of(
            StringSubstitutor(
                    java.util.Map.of<String, String>("raw_table_id", streamId.rawTableId(QUOTE))
                )
                .replace(
                    """
